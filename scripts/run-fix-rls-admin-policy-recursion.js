/**
 * Fix recursive admin RLS policies by using is_admin().
 * Run: node scripts/run-fix-rls-admin-policy-recursion.js
 */
...
async function main() {
  ...
  const sqlPath = path.join(__dirname, "..", "database", "fix-rls-admin-policy-recursion.sql")
  ...
  try {
    ...
    console.log("OK: recursive admin RLS policies fixed.")
  } catch (err) {
